...
function addProd(obj) {
    ...
    if ($('input[name="quant[' + id + ']"]').length == 0) {
        var tr =
                ...
                '</td>' +
                '<td>R$ ' + price + '</td>' +
                '<td id="subtotal" class="subtotal" name="subtotal">R$ ' + price + '</td>' +
        ...
        if (datatype != '') {
            $.ajax({
                ...
                success: function (json) {
                    ...
                    for (var i in json) {
                        html += '<div class="si"><a href="javascript:;" onclick="addProd(this)" data-id="'
                                + json[i].id + '" data-price="' + json[i].price + '" data-name="'
                                + json[i].name + '">' + json[i].name + ' - R$ ' + json[i].price + '</a></div>';
                    }
