...
class Sales extends model {
    ...
    public function addSale($id_company, $id_client, $id_user, $quant, $status, $total_price) {
        ...
        foreach ($quant as $id_prod => $quant_prod) {
            $sql = $this->db->prepare("SELECT price FROM inventory WHERE "
                    . "id = :id AND "
            ...
            if ($sql->rowCount() > 0) {
                ...
                $sqlp = $this->db->prepare("INSERT INTO sales_products SET "
                        . "id_company = :id_company, "
                        . "id_sale = :id_sale, "
    ...
    public function getSoldProducts($period1, $period2, $id_company) {
        ...
        if ($sql->rowCount() > 0) {
            ...
            foreach ($sql->fetchAll() as $sale_item) {
                $product[] = $sale_item['id'];
            }

            $sql = $this->db->query("SELECT COUNT(*) as total FROM sales_products WHERE id_sale IN (" . implode(',', $product) . ")");
            $number = $sql->fetch();
            $int = $number['total'];
        }
    ...
    public function getAllInfo($id_sale, $id_company) {
        ...
        if ($sql->rowCount() > 0) {
            ...
            foreach ($array['products'] as $pkey => $pval) {
                ...
                $array['products'][$pkey]['c'] = $inventory->getInfo($pval['id_product'], $id_company);
            }
        }
