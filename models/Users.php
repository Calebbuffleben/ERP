...
class Users extends model {
    ...
    public function getName() {
        if (isset($this->userInfo['user_name'])) {
    ...
    public function findInGroup($id) {
    ...
    public function getList($id_company) {
        ...
        $sql = $this->db->prepare("SELECT users.id, users.user_name, permission_groups.name FROM users "
                . "LEFT JOIN permission_groups ON permission_groups.id = users.id_group WHERE users.id_company = :id_company");
        $sql->bindValue(":id_company", $id_company);
        $sql->execute();
    ...
    public function add($email, $user_name, $pass, $group, $id_company) {
        $sql = $this->db->prepare("SELECT COUNT(*) as c FROM users WHERE user_name = :user_name");
        ...
        if ($row['c'] == '0') {
            ...
            $sql = $this->db->prepare("INSERT INTO users SET email = :user_name, user_name = :user_name, password = :password, id_group = :id_group, id_company = :id_company");
            $sql->bindValue(":email", $email);
            $sql->bindValue(":user_name", $name);
